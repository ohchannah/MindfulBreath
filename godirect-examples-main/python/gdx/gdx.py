...
class gdx:
            ...
            if type(sensors) == list:
                ...
                if isinstance(sensors[0], list):
                    ...
                    if len(sensors) != len(gdx.devices):
                        print(
                            "the sensor parameter in select_sensors() does not match number of devices"
                        )
                        gdx.devices = []
                        return
                    else:
            ...
            for sensor_selected in sensors_selected_by_user:
                ...
                if sensor_selected in all_sensor_numbers:
                    ...
                    print(
                        "The value ",
                        sensor_selected,
                        " in select_sensors() is not valid",
                    )
    ...
    def start(self, period=None):
        ...
        # This is not a vpython program.
            ...
            if period < 10:
                input(
                    "Be aware that sampling at a period less than 10ms may be problemeatic. Press Enter to continue "
                )
        ...
        if (
            gdx.vpython == True
            and gdx.vpython_buttons == True
            and gdx.vp_first_start == True
        ):
    ...
    def read(self):
        ...
        if gdx.buffer:
            for i in range(len(gdx.buffer)):
                if gdx.buffer[i]:  # 버퍼가 비어 있지 않은 경우만 pop
                    pop_values = gdx.buffer[i].pop(0)
                    retvalues.append(pop_values)
                else:
                    print(f"Buffer at index {i} is empty.")
            if gdx.buffer and not gdx.buffer[0]:
                gdx.buffer = []

        # The buffer is empty, so take readings from the sensor to get retvalues
        else:
            gdx.buffer = []
            for i, device in enumerate(gdx.devices):
                if device.read():
                    sensors = gdx.enabled_sensors[i]
                    if sensors:
                        for sensor in sensors:
                            values[:] = sensor.values
                            if values:  # 값이 있는 경우만 pop
                                ...
                                print(f"Sensor {sensor} returned empty values.")
                            values = []
        ...
        while i < len(gdx.devices):
            ...
            for x in sensors:
                ...
                available_sensors.append(
                    [number, description, units, incompatible_sensors]
                )
    ...
    def vp_vernier_canvas(
        self, buttons=True, slider=True, meters=True, chart=False, cvs=True
    ):
